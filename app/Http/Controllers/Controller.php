...
class Controller extends BaseController
{
    ...
    public static function updateJobMeta($id, $meta_key = "", $meta_value)
    {
        //
        try {
            JobMeta::updateOrCreate(
                [
                    'job_id' => $id,
                    'key' => $meta_key,
                ],
    ...
    public static function updateJobAllMeta($job_id, $meta_key_value = [])
    {
        //
        $key_value = [];
        foreach ($meta_key_value as $key => $value) {
    ...
    public static function getJobMeta($id, $key = "", $status = false)
    {
        if (empty($key)) {
            // 
            $job_details_add = JobMeta::where('job_id', $id)->select('key', 'value')
                ->pluck('value', 'key')
                ->toArray();
            return $job_details_add;
        } else {
            //
            if ($status) {
                // 
                $job_details_add = JobMeta::where('job_id', $id)->where('key', $key)->first();
                if (!empty($job_details_add))
                    return $job_details_add->value;
                else
                    return "";
            } else {
                $job_details_add = JobMeta::where('job_id', $id)->where('key', $key)->select('key', 'value')
                    ->pluck('value', 'key')
                    ->toArray();
                return $job_details_add;
            }
